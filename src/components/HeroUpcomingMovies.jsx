...
export default function HeroUpcomingMovies() {
  const navigate = useNavigate();
  const { isWatchlistEmpty, setIsWatchlistEmpty } = useWatchlistEmptyCheck();

  const { data, isLoading, error } = useQuery({
    queryKey: ["heroUpcomingMovies", 1],
    queryFn: () => heroUpcomingMovies(1),
    staleTime: 1000 * 60 * 5,
    cacheTime: 1000 * 60 * 6,
  ...
  return (
    <section className="bg-[#F7EBDF]">
      <div className="flex justify-between items-center px-4 pt-4 mt-10 max-sm:mt-[-1px]">
        <h2 className="text-xl font-bold text-black max-sm:uppercase max-sm:text-sm">Upcoming Movies</h2>
        <p
